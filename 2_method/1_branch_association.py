...
f = [interp1d(
    r, tdoa[:, k], kind="linear", fill_value=np.nan, bounds_error=False
) for k in range(N)]

fs = [interp1d(
    tdoa[:, k], r, kind="linear", fill_value=np.nan, bounds_error=False
) for k in range(N)]
...
fig, axes = plt.subplots(nrows=2, sharex=True, gridspec_kw=dict(
